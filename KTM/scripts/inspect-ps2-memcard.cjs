...
const tmpBundle = path.join(
  os.tmpdir(),
  `ktm-ps2mc-bundle-${process.pid}.cjs`
);
...
const testParseAndExport = async (tmp, icon, test) => {
  ...
  const outPath = path.join(os.tmpdir(), `ktm-synth-${process.pid}.psu`);
...
const testImportCase = async (withEcc, psu, icon, test) => {
  ...
  const target = path.join(
    os.tmpdir(),
    `ktm-import-${process.pid}-${label}.ps2`
  );
...
const runSelfTest = async () => {
  ...
  const tmp = path.join(os.tmpdir(), `ktm-synth-card-${process.pid}.ps2`);
