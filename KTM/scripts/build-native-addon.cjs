...
const outputDir = path.join(projectRoot, "ktm-native");
const outputNodePath = path.join(outputDir, "ktm-native.node");

const sourceLibraryNameByPlatform = {
  linux: "libktm_native.so",
  darwin: "libktm_native.dylib",
  win32: "ktm_native.dll",
};
...
const build = async () => {
  ...
  console.log("Building ktm-native Rust addon...");
  ...
  console.log(`KTM native addon ready at ${outputNodePath}`);
};
