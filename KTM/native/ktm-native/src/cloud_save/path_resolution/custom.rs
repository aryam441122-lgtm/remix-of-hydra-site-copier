...
mod tests {
    ...
    fn decodes_portable_and_absolute_paths_for_each_platform() {
        assert_eq!(
            decode_custom_path("<custom><windows><winAppData>/KTM/Saves"),
            Some(Ok(DecodedCustomPath {
                platform: "windows".to_string(),
                path: "<winAppData>/KTM/Saves".to_string()
            }))
        );
        assert_eq!(
    ...
    fn rejects_relative_traversing_and_wrong_platform_tokens() {
        ...
        assert!(decode_custom_path("<custom><windows>c:/Saves")
            .unwrap()
            .is_err());
        assert!(decode_custom_path("<custom><windows>C:/Users/KTM/Saves")
            .unwrap()
            .is_err());
        assert!(decode_custom_path("<custom><linux>/home/ktm/Saves")
            .unwrap()
            .is_err());
    }
}
