...
fn resolve_standard_path(raw_path: &str, context: &PathResolutionContext) -> ResolvedPath {
    ...
    if context.windows_compatibility {
        if let Some(prefix) = &context.wine_prefix_path {
            // This is the prefix KTM passes to the launcher. Other Proton
...
mod tests {
    ...
    fn uses_active_launcher_prefix_without_scanning_other_compatdata() {
        let input = ResolveSaveRulesInput {
            ...
            documents_dir: None,
            app_data_dir: None,
            wine_prefix_path: Some("/ktm/prefix".into()),
        ...
        assert_eq!(
            paths[0],
            "/ktm/prefix/drive_c/users/steamuser/AppData/Roaming/Game"
        );
        assert_eq!(
            paths[1],
            "/ktm/prefix/drive_c/users/victor/AppData/Roaming/Game"
        );
        assert_eq!(
            paths[2],
            "/ktm/prefix/drive_*/users/*/AppData/Roaming/Game"
        );
        assert_eq!(
            paths[3],
            "/ktm/prefix/drive_c/users/steamuser/Application Data/Game"
        );
        assert!(paths.iter().all(|path| path.starts_with("/ktm/prefix/")));
    ...
    fn custom_store_root_stays_on_the_host_across_windows_compatibility() {
        ...
        assert!(root
            .paths
            .iter()
            .all(|candidate| !candidate.path.starts_with("/ktm/prefix")));
        assert_eq!(
            base.paths[0].path,
    ...
    fn ignores_wine_prefix_for_native_linux_executable() {
        ...
        assert!(result
            .paths
            .iter()
            .all(|path| !path.path.starts_with("/ktm/prefix")));
    }
}
