...
mod tests {
    ...
    #[test]
    fn scans_ktm_launcher_prefix_without_mixing_steam_compatdata() {
        let temp = tempdir().unwrap();
        ...
        let ktm_prefix = temp.path().join("ktm-prefix");
        let ktm_save = ktm_prefix
        ...
        fs::create_dir_all(&ktm_save).unwrap();
        fs::write(proton_save.join("UserSettings.json"), b"steam-old").unwrap();
        fs::write(ktm_save.join("UserSettings.json"), b"ktm-active").unwrap();
        ...
        let rules = resolve_save_rules(ResolveSaveRulesInput {
            ...
            wine_prefix_path: Some(ktm_prefix.display().to_string()),
            steam_path: None,
            rules: vec![CloudSaveRule {
                rule_id: "test-rule".into(),
                kind: "dir".into(),
    ...
    fn active_empty_prefix_does_not_fall_back_to_host_home() {
        let temp = tempdir().unwrap();
        let active_prefix = temp.path().join("ktmlauncher/wine-prefixes/953490");
