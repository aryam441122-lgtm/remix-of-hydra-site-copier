...
function UserProfileContent({
  ...
  const handleCopy = (event: MouseEvent<HTMLButtonElement>) => {
    ...
    if (isCopied) return;
    setIsCopied(true);
    globalThis.window.electron.clipboard.writeText(profileCode).catch(() => {});
  ...
  const content = (
    ...
    >
      ...
      <div className="user-profile-content__info">
        ...
        <button
          className="user-profile-content__info__profile-code"
          onClick={handleCopy}
        >
          {profileCode}
          {isCopied ? (
            <CheckIcon
              size={14}
              className="user-profile-content__info__profile-code__icon"
            />
          ) : (
            <CopyIcon
              size={14}
