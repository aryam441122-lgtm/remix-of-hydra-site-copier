...
export function GameCloudSettingsTab({
  ...
  const loadArtifacts = useCallback(async () => {
    ...
    const result = await globalThis.window.electron.ktmApi
  ...
  const handleToggleArtifactFreeze = useCallback(
    async (artifactId: string, freeze: boolean) => {
      ...
      try {
        await globalThis.window.electron.ktmApi.put(
    ...
    async (artifactId: string) => {
      ...
      try {
        if (isEmulationGame) {
          ...
          await globalThis.window.electron.ktmApi.delete<{ ok: boolean }>(
  ...
  return (
    <VerticalFocusGroup className="game-cloud-settings-tab">
      {isEmulationGame ? (
        ...
        <SettingsSection
          className="game-cloud-settings-tab__section"
          title={tGameDetails("settings_category_ktm_cloud")}
