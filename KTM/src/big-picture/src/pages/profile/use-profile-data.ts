...
async function getUserLibrary(
  ...
  return globalThis.window.electron.ktmApi.get<UserLibraryResponse>(
...
export function useProfileBadges() {
  ...
  useEffect(() => {
    globalThis.window.electron.ktmApi
...
export function useExternalProfile(targetUserId: string | undefined) {
  ...
  const reload = useCallback(async (profileId: string) => {
    const profile = await globalThis.window.electron.ktmApi.get<UserProfile>(
      `/users/${profileId}`
    );
...
export function useProfileSouvenirs(
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      const response =
        await globalThis.window.electron.ktmApi.get<SouvenirsResponse | null>(
          buildUserSouvenirsPath({
            userId: targetUserId,
            skip,
