...
const ktmIconUrl = new URL("../../assets/ktm-icon.svg", import.meta.url)
  .href;
        ...
        <ProfileStatPair
          ...
          rightIcon={
            <img
              src={ktmIconUrl}
              alt=""
              className="profile-page__stat-ktm-icon"
              draggable={false}
            />
          }
  ...
  return (
    <div className="profile-page__achievements-lock-frame">
      ...
      <div className="profile-page__achievements-lock-overlay">
        ...
        <p>
          {isOwnProfile
            ? "You need KTM Cloud in order to display achievements in your own profile."
            : "This user is required to have KTM Cloud in order to display achievements in his profile."}
        </p>
      </div>
    </div>
  );
...
function getProfileNavigation({
  ...
  const firstSocialFocusId = firstAchievementFocusId;
...
function ProfileContent({ userId }: Readonly<ProfileContentProps>) {
  ...
  const handleBlockUser = useCallback(
    async (actionUserId: string) => {
      ...
      try {
        ...
        // Keep the current profile visible if the action fails.
      } finally {
        setIsPerformingProfileAction(false);
      }
    },
    [blockUser, navigate, profileUser]
  );
  ...
  const handleSouvenirLike = useCallback(
    async (souvenir: ProfileSouvenir) => {
      ...
      try {
        await globalThis.window.electron.ktmApi.post(
  ...
  const handleSouvenirVisibilityChange = useCallback(
    async (souvenir: ProfileSouvenir) => {
      ...
      try {
        await globalThis.window.electron.ktmApi.patch(
  ...
  const handleSouvenirReport = useCallback(
    async (souvenir: ProfileSouvenir, values: SouvenirReportValues) => {
      ...
      try {
        const response = await globalThis.window.electron.ktmApi.postResponse(
  ...
  const externalProfileActions = useMemo<ProfileHeroAction[]>(() => {
    ...
    return getExternalProfileActions(externalProfile, (actionUserId) => {
      handleBlockUser(actionUserId).catch(() => {});
    });
  }, [externalProfile, handleBlockUser, profileUser]);
