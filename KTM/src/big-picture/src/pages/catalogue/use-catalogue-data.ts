  ...
  useEffect(() => {
    ...
    const loadMetadata = async () => {
      const [
        ...
        globalThis.window.electron.ktmApi.get<string[]>(
          "/catalogue/steam/genres",
          { params: { language: "en" }, needsAuth: false }
        ),
        globalThis.window.electron.ktmApi.get<Record<string, number>>(
        ...
        globalThis.window.electron.ktmApi.get<LaunchboxFiltersResponse>(
  ...
  const fetchCataloguePage = useCallback(
    async (
      ...
      try {
        ...
        const response =
          await globalThis.window.electron.ktmApi.post<SearchGamesResponseData>(
            "/catalogue/search",
            {
              data: {
                ...payload,
