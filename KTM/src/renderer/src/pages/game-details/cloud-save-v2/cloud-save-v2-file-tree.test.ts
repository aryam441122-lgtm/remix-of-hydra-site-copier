...
const customPath: CloudSaveCustomPath = {
  ...
  path: "C:\\Users\\KTM\\Documents\\Game",
  platform: "windows",
};
...
describe("cloud save V2 local file tree", () => {
  it("formats Windows extended paths without changing Unix paths", () => {
    assert.equal(
      formatCloudSaveV2LocalPath("//?/C:/Users/KTM/Saves/slot.dat"),
      "C:\\Users\\KTM\\Saves\\slot.dat"
    );
    assert.equal(
      formatCloudSaveV2LocalPath("\\\\?\\C:\\Users\\KTM\\Saves"),
      "C:\\Users\\KTM\\Saves"
    );
    assert.equal(
    ...
    assert.equal(
      formatCloudSaveV2LocalPath("/home/ktm/saves/slot.dat"),
      "/home/ktm/saves/slot.dat"
    );
  });
  ...
  it("uses real Windows paths and normalizes mixed relative separators", () => {
    const [root] = buildCloudSaveV2LocalFileTree([
      localFile(
        ...
        "C:\\Users\\KTM\\Saves\\profiles\\one\\slot.dat"
      ),
    ]);

    assert.equal(root.name, "C:\\Users\\KTM\\Saves");
    assert.equal(root.localDirectoryPath, "C:\\Users\\KTM\\Saves");
  ...
  it("preserves Unix and UNC roots belonging to different rules", () => {
    const roots = buildCloudSaveV2LocalFileTree([
      ...
      localFile("rule-a", "slot.dat", "/home/ktm/saves/slot.dat"),
    ]);

    assert.deepEqual(
      roots.map((root) => root.name),
      ["/home/ktm/saves", "\\\\server\\share"]
    );
  ...
  it("shows a remote-only legacy custom location as removable", () => {
    const rawPath = "<custom><windows>C:/Users/KTM/AppData/Roaming/Game";
...
describe("cloud save V2 comparison tree", () => {
  ...
  it("marks a remote-only legacy custom root as removable", () => {
    const rawPath = "<custom><linux>/home/ktm/.local/share/game";
  ...
  it("keeps a registered unresolved binding visible without comparisons", () => {
    ...
    unresolved.pathHint = "C:\\Users\\KTM\\Documents\\Game";
