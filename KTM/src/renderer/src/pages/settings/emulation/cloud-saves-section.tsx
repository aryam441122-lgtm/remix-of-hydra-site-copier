...
import ktmSaveCard from "@renderer/assets/emulation/icons/ktm-save-card.png";
...
export function CloudSavesSection({ config, refreshKey }: Readonly<Props>) {
  ...
  const { t: tKTMCloud } = useTranslation("ktm_cloud");
        ...
        <div className="emulator-detail__cloud-locked">
          ...
          <div className="emulator-detail__cloud-locked-overlay">
            ...
            <p className="emulator-detail__cloud-locked-title">
              {tKTMCloud("ktm_cloud_feature_found")}
            </p>
            <Button
              theme="outline"
              onClick={() => showKTMCloudModal("backup")}
            >
              <KTMIcon className="emulator-detail__cloud-locked-ktm" />
              <span>{tKTMCloud("learn_more")}</span>
            </Button>
          </div>
        </div>
      </section>
