...
export default function Notifications() {
  ...
  const fetchBadges = useCallback(async () => {
    try {
      ...
      const badgesResponse = await window.electron.ktmApi.get<Badge[]>(
  ...
  const fetchApiNotifications = useCallback(
    async (
      ...
      try {
        setIsLoading(true);
        const response =
          await window.electron.ktmApi.get<NotificationsResponse>(
    ...
    async (id: string, source: "api" | "local") => {
      try {
        if (source === "api") {
          ...
          await window.electron.ktmApi.patch(
    ...
    async (id: string, source: "api" | "local") => {
      try {
        if (source === "api") {
          ...
          await window.electron.ktmApi.delete(
  ...
  const handleClearAll = useCallback(async () => {
    ...
    try {
      ...
      if (userDetails) {
        await window.electron.ktmApi.delete(`/profile/notifications/all`, {
          needsAuth: true,
        });
      }
