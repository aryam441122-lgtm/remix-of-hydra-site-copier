...
export function useGameActions(game: GameContextMenuGame) {
  ...
  const handlePlayGame = async () => {
    ...
    if (!canPlay) {
      ...
      globalThis.dispatchEvent(
        new CustomEvent("ktm:openRepacks", {
    ...
    if (isClassics) {
      ...
      if (needsModal) {
        ...
        globalThis.dispatchEvent(
          new CustomEvent("ktm:openDiscSelection", {
            detail: { objectId: game.objectId },
          })
        );
        return;
    ...
    try {
      ...
      try {
        window.dispatchEvent(
          new CustomEvent("ktm:game-favorite-toggled", {
  ...
  const handleOpenGameOptions = (initialCategory?: GameSettingsCategoryId) => {
    ...
    try {
      window.dispatchEvent(
        new CustomEvent("ktm:openGameOptions", {
          detail: { objectId: game.objectId },
        })
      );
    } catch (e) {
  ...
  const handleTogglePin = async () => {
    try {
      ...
      try {
        window.dispatchEvent(
          new CustomEvent("ktm:game-pin-toggled", {
            detail: { shop: game.shop, objectId: game.objectId },
          })
        );
      } catch {
  ...
  const handleRemoveFromLibrary = async () => {
    try {
      ...
      try {
        window.dispatchEvent(
          new CustomEvent("ktm:game-removed-from-library", {
            detail: { shop: game.shop, objectId: game.objectId },
          })
        );
      } catch {
  ...
  const handleRemoveFiles = async () => {
    try {
      ...
      try {
        window.dispatchEvent(
          new CustomEvent("ktm:game-files-removed", {
            detail: { shop: game.shop, objectId: game.objectId },
          })
        );
      } catch {
