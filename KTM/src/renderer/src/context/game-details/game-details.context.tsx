...
export function GameDetailsContextProvider({
  ...
  useEffect(() => {
    ...
    window.addEventListener("ktm:openRepacks", handler as EventListener);

    return () => {
      window.removeEventListener("ktm:openRepacks", handler as EventListener);
    };
  }, [objectId]);
  ...
  useEffect(() => {
    ...
    window.addEventListener("ktm:openGameOptions", handler as EventListener);
  ...
  useEffect(() => {
    ...
    const fetchDownloadSources = async () => {
      try {
        ...
        const downloads = await window.electron.ktmApi.get<GameRepack[]>(
        ...
        const results = await fetchHosterAvailability(
          downloadOptions,
          (url, data) =>
            window.electron.ktmApi.post(url, { data, needsAuth: false })
        );
