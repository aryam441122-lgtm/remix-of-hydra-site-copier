...
import { CloudSubscriptionModal } from "./pages/shared-modals/ktm-cloud/cloud-subscription-modal";
...
export function App() {
  ...
  const { hideKTMCloudModal, isKTMCloudModalVisible, ktmCloudFeature } =
    useSubscription();
  ...
  useEffect(() => {
    window.electron.getActiveClassicsImport().then((snapshot) => {
      if (snapshot) dispatch(ktmteClassicsScan(snapshot));
    });
  ...
  useEffect(() => {
    window.electron.getActiveRetroArchImport().then((snapshot) => {
      if (snapshot) dispatch(ktmteRetroArchScan(snapshot));
    });
  ...
  useEffect(() => {
    new MutationObserver(() => {
      const modal = document.body.querySelector("[data-ktm-dialog]");
  ...
  return (
    <>
      ...
      <CloudSubscriptionModal
        visible={isKTMCloudModalVisible}
        onClose={hideKTMCloudModal}
        feature={ktmCloudFeature || undefined}
      />
