...
import { KTMApi, WindowManager } from "@main/services";
...
export const patchUserProfile = async (updateProfile: UpdateProfileRequest) => {
  return KTMApi.patch<UserProfile>("/profile", updateProfile);
};
...
const uploadImage = async (
  ...
  const response = await KTMApi.post<{ presignedUrl: string }>(
    `/presigned-urls/${type}`,
    {
...
const updateProfile = async (
  ...
  // Notify every window (each has its own store) so
