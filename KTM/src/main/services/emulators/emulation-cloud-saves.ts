 ...
 * through `KTMApi` (auth + subscription enforced), while the raw artifact
 ...
 *
 * Every call requires an active KTM Cloud subscription.
 */
...
export const uploadEmulationSave = async (
  ...
  const { id, uploadUrl } = await KTMApi.post<{
    id: string;
    uploadUrl: string;
  }>(
  ...
  return KTMApi.post<EmulationCloudSave>(
...
export const listEmulationSaves = async (
  ...
  const response = await KTMApi.get<EmulationCloudSave[]>(
...
export const downloadEmulationSaveBytes = async (
  ...
  const { downloadUrl } = await KTMApi.post<{ downloadUrl: string }>(
...
export const deleteEmulationSave = async (id: string): Promise<void> => {
  await KTMApi.delete(`/profile/emulation-saves/${id}`, SUB);
};
...
export const updateEmulationSave = async (
  ...
  return KTMApi.put<EmulationCloudSave>(
