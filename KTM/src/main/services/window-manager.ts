...
export class WindowManager {
  ...
  private static bindAuthNavigation(
    ...
    contents.on("will-navigate", (_event, url) => {
      if (url.startsWith("ktmlauncher://auth")) {
        closeWindow();

        KTMApi.handleExternalAuth(url);
        return;
      }

      if (url.startsWith("ktmlauncher://update-account")) {
        closeWindow();
  ...
  public static openAuthWindow(page: AuthPage, searchParams: URLSearchParams) {
    ...
    /* KTM accounts are handled locally by KTM Cloud, no remote auth page. */
    void page;
    void searchParams;

    if (process.platform === "linux") {
      this.openLinuxAuthWindow(parentWindow);
      return;
    }
    ...
    void this.loadWindowURL(authWindow, "auth-window");
  ...
  private static openLinuxAuthWindow(parentWindow: Electron.BrowserWindow) {
    const authWindow = new BrowserWindow({
    ...
    this.bindAuthNavigation(authWindow.webContents, () => {
