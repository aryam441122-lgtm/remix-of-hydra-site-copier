...
export class DeckyPlugin {
  ...
  private static async getDeckyReleaseInfo(): Promise<DeckyReleaseInfo> {
    ...
    try {
      const response = await KTMApi.get<DeckyReleaseInfo>(
        "/decky/release",
        {},
        { needsAuth: false }
      );
  ...
  private static async extractPlugin(zipPath: string): Promise<string> {
    logger.log("Extracting KTM Decky plugin...");

    const tempDir = SystemPath.getPath("temp");
    const extractPath = path.join(tempDir, "ktm-decky-plugin");
  ...
  private static async installPluginWithSudo(
    ...
    const sourcePath = path.join(extractPath, "KTM");

    return new Promise((resolve, reject) => {
      const command = `mkdir -p "${DECKY_PLUGINS_LOCATION}" && rm -rf "${KTM_DECKY_PLUGIN_LOCATION}" && cp -r "${sourcePath}" "${KTM_DECKY_PLUGIN_LOCATION}" && chown -R ${username}: "${DECKY_PLUGINS_LOCATION}"`;
  ...
  private static async installPluginWithoutSudo(
    ...
    if (fs.existsSync(KTM_DECKY_PLUGIN_LOCATION)) {
      await fs.promises.rm(KTM_DECKY_PLUGIN_LOCATION, {
        recursive: true,
        force: true,
      });
    }

    await fs.promises.cp(sourcePath, KTM_DECKY_PLUGIN_LOCATION, {
      recursive: true,
    });
  ...
  public static async checkAndUpdateIfOutdated(): Promise<void> {
    if (!fs.existsSync(KTM_DECKY_PLUGIN_LOCATION)) {
      logger.log("KTM Decky plugin not installed, skipping update check");
      return;
    }
    ...
    try {
      if (!fs.existsSync(packageJsonPath)) {
        logger.log(
          "KTM Decky plugin package.json not found, skipping update"
        );
        return;
      }
      ...
      if (isOutdated) {
        logger.log(
          `KTM Decky plugin is outdated. Current: ${currentVersion}, Expected: ${releaseInfo.version}. Updating...`
        );

        await this.updatePlugin();
        logger.log("KTM Decky plugin updated successfully");
      } else {
        logger.log(`KTM Decky plugin is up to date (${currentVersion})`);
      }
    } catch (error) {
      logger.error(`Error checking/updating KTM Decky plugin: ${error}`);
    }
  }
  ...
  public static async checkPluginVersion(): Promise<{
    ...
    try {
      ...
      if (!fs.existsSync(KTM_DECKY_PLUGIN_LOCATION)) {
        logger.log("KTM Decky plugin folder not found, installing...");
      ...
      try {
        if (!fs.existsSync(packageJsonPath)) {
          logger.log(
            "KTM Decky plugin package.json not found, installing..."
          );
        ...
        if (isOutdated) {
          logger.log(
            `KTM Decky plugin is outdated. Current: ${currentVersion}, Expected: ${releaseInfo.version}`
          );
        ...
        logger.error(`Error checking KTM Decky plugin version: ${error}`);
        return {
          exists: false,
          outdated: true,
          currentVersion: null,
