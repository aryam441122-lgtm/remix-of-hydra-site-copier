...
before(() => {
  workingDirectory = fs.mkdtempSync(
    path.join(os.tmpdir(), "ktm-notification-icon-test-")
  );
});
...
describe("buildDownloadFileName", () => {
  it("produces a name free of characters that are illegal on Windows", () => {
    const fileName = buildDownloadFileName(
      "https://cdn.ktm/profile-images/avatar.webp?v=2&token=a/b"
    );
  ...
  it("keeps the source extension", () => {
    assert.ok(
      buildDownloadFileName("https://cdn.ktm/x/avatar.webp").endsWith(".webp")
    );
  });

  it("drops the query string from the extension", () => {
    assert.ok(
      buildDownloadFileName("https://cdn.ktm/x/avatar.webp?v=2").endsWith(
        ".webp"
      )
    );
  });
  ...
  it("gives different urls different names", () => {
    assert.notEqual(
      buildDownloadFileName("https://cdn.ktm/a/avatar.webp"),
      buildDownloadFileName("https://cdn.ktm/b/avatar.webp")
    );
  });

  it("handles urls with no basename", () => {
    const fileName = buildDownloadFileName("https://cdn.ktm/");
