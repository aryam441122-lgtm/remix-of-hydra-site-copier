...
export interface KTMApiOptions {
  needsAuth?: boolean;
...
export class KTMApi {
  ...
  private static userAuth: KTMApiUserAuth = {
    authToken: "",
    refreshToken: "",
  ...
  /**
   * The legacy remote cloud backend is disabled: KTM now uses KTM Cloud
   * (Lovable Cloud) for accounts. Legacy endpoints are migrated gradually.
   */
  ...
  static async get<T = any>(
    url: string,
    params?: any,
    options?: KTMApiOptions
  ) {
    await this.validateOptions(options);

    const headers = {
      ...this.getAxiosConfig().headers,
      "KTM-If-Modified-Since": options?.ifModifiedSince?.toUTCString(),
  ...
  static async delete<T = any>(url: string, options?: KTMApiOptions) {
    await this.validateOptions(options);
  ...
  static async checkDownloadSourcesChanges(
    ...
    logger.info("KTMApi.checkDownloadSourcesChanges called with:", {
      downloadSourceIds,
    ...
    try {
      ...
      logger.info(
        "KTMApi.checkDownloadSourcesChanges completed successfully:",
        result
      );
      return result;
    } catch (error) {
      logger.error("KTMApi.checkDownloadSourcesChanges failed:", error);
      throw error;
    }
  }
}
