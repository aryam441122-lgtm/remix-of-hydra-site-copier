...
const wineContext = (
  userName: string,
  winePrefixPath = `/home/${userName}/.local/share/ktm/prefix`
): CloudSaveCustomPathContext => ({
...
describe("cloud save custom path codec", () => {
  it("encodes known directories with the same portable tokens as normal saves", () => {
    assert.deepEqual(
      encodeCloudSaveCustomPath(
        "c:\\Users\\KTM\\AppData\\Roaming\\Game\\",
        windowsContext("KTM")
      ),
      {
        rawPath: "<custom><windows><winAppData>/Game",
        path: "C:/Users/KTM/AppData/Roaming/Game",
        platform: "windows",
      }
    );
    assert.equal(
      encodeCloudSaveCustomPath(
        "/home/ktm/.local/share/game/",
        linuxContext("ktm")
      ).rawPath,
    ...
    assert.equal(
      encodeCloudSaveCustomPath(
        "/Users/ktm/Library/Application Support/Game",
        macContext("ktm")
      ).rawPath,
  ...
  it("binds a remote custom path identity to an explicitly chosen folder", () => {
    ...
    assert.equal(binding.path, "D:/KTM Saves/Game");
  ...
  it("uses one Windows identity on native Windows and Wine", () => {
    ...
    assert.equal(
      ...
      "/home/maria/.local/share/ktm/prefix/drive_c/users/steamuser/AppData/Roaming/Game/Saves"
    );
  });
  ...
  it(
    ...
    async () => {
      const directory = await fs.mkdtemp(
        path.join(os.tmpdir(), "ktm-custom-active-wine-profile-")
      );
...
`
  ...
  it("leaves Windows custom paths unavailable without a valid USERPROFILE", async () => {
    const directory = await fs.mkdtemp(
      path.join(os.tmpdir(), "ktm-custom-missing-wine-profile-")
    );
    const prefix = path.join(directory, "prefix");
  ...
  it("encodes a selected Wine profile directory as a Windows custom path", () => {
    ...
    const selectedPath =
      "/home/rodrigo/.local/share/ktm/prefix/drive_c/users/steamuser/AppData/Roaming/Game";
  ...
  it("uses only the active Wine profile", () => {
    ...
    assert.equal(
      decodeCloudSaveCustomPath("<custom><windows><winDocuments>/Game", context)
        .path,
      "/home/rodrigo/.local/share/ktm/prefix/drive_c/users/steamuser/Documents/Game"
    );
    assert.throws(() =>
      encodeCloudSaveCustomPath(
        "/home/rodrigo/.local/share/ktm/prefix/drive_c/users/player-two/Documents/Game",
        context
      )
    );
  });
  ...
  it("keeps Linux Wine paths platform-specific instead of aliasing them", () => {
    const linuxRawPath =
      "<custom><linux><home>/.local/share/ktm/prefix/drive_c/users/steamuser/AppData/Roaming/Game";
    ...
    assert.equal(
      ...
      "/home/maria/.local/share/ktm/prefix/drive_c/users/steamuser/AppData/Roaming/Game"
    );
  });
  ...
  it("keeps explicit absolute paths platform-specific", () => {
    ...
    assert.throws(() =>
      decodeCloudSaveCustomPath(rawPath, windowsContext("KTM"))
    );
  });
  ...
  it("marks bare absolute paths as legacy and never decodes them", () => {
    ...
    assert.throws(() =>
      decodeCloudSaveCustomPath(legacyWindows, windowsContext("KTM"))
    );
    assert.throws(() =>
      decodeCloudSaveCustomPath(legacyLinux, linuxContext("ktm"))
    );
  });

  it("encodes new non-portable absolute paths with the explicit marker", () => {
    assert.equal(
      encodeCloudSaveCustomPath("D:/Saves/Game", windowsContext("KTM"))
        .rawPath,
      "<custom><windows><absolute>D:/Saves/Game"
    );
  });
  ...
  it("rejects unknown platforms, relative paths, traversal and roots", () => {
    assert.throws(() =>
      decodeCloudSaveCustomPath(
        "<custom><bsd>/home/ktm/game",
        linuxContext("ktm")
      )
    );
    for (const rawPath of [
      "<custom><linux>relative/game",
      "<custom><linux><absolute>/home/ktm/../game",
      "<custom><linux><home>/../game",
      "<custom><linux><absolute>/",
    ]) {
      assert.throws(() =>
        decodeCloudSaveCustomPath(rawPath, linuxContext("ktm"))
      );
    }
    for (const rawPath of [
      "<custom><windows>c:/Users/KTM/Game",
  ...
  it("isolates persisted paths by KTM user, shop and objectId", () => {
    assert.notEqual(
  ...
  it("canonicalizes an existing selected directory", async () => {
    const directory = await fs.mkdtemp(
      path.join(os.tmpdir(), "ktm-custom-path-")
    );
  ...
  it(
    ...
    async () => {
      const directory = await fs.mkdtemp(
        path.join(os.tmpdir(), "ktm-custom-wine-symlink-")
      );
