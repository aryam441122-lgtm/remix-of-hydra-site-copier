...
const getForegroundWindow = async (): Promise<ForegroundWindow | null> => {
  try {
    if (process.platform === "win32") {
      const script = [
        'Add-Type -TypeDefinition \'using System; using System.Runtime.InteropServices; public static class KTMForegroundWindow { [DllImport("user32.dll")] public static extern IntPtr GetForegroundWindow(); [DllImport("user32.dll")] public static extern uint GetWindowThreadProcessId(IntPtr hWnd, out uint processId); }\'',
        "$handle = [KTMForegroundWindow]::GetForegroundWindow()",
        "$processId = 0",
        "[void][KTMForegroundWindow]::GetWindowThreadProcessId($handle, [ref]$processId)",
