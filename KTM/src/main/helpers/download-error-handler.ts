...
const KTM_UNLOCK_DOWNLOADERS = new Set<Downloader>([
  Downloader.Datanodes,
  Downloader.VikingFile,
]);

const handleKTMUnlockError = (
  err: AxiosError,
  downloader: Downloader
): DownloadErrorResult | null => {
  if (!KTM_UNLOCK_DOWNLOADERS.has(downloader)) return null;
...
const handleAxiosError = (
  ...
  const ktmUnlockResult = handleKTMUnlockError(err, downloader);
  if (ktmUnlockResult) return ktmUnlockResult;
...
export const handleDownloadError = (
  ...
  if (
    ...
    KTM_UNLOCK_DOWNLOADERS.has(downloader)
  ) {
    return { ok: false, error: DownloadError.HosterUnlockLoginRequired };
  }
