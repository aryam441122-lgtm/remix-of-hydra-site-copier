...
import { getKTMExecutablePath } from "./ktm-executable-path";

export { getKTMExecutablePath } from "./ktm-executable-path";
...
export const buildRunDeepLink = (shop: GameShop, objectId: string) => {
  ...
  return `ktmlauncher://run?${query.toString()}`;
};
...
export const getKTMShortcutTarget = (deepLink: string) => {
  const portableLauncherPath = refreshPortableShortcutLauncher();
  if (!portableLauncherPath) {
    return {
      executablePath: getKTMExecutablePath(),
