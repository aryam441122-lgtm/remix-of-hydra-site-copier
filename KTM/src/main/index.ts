...
const initializeApp = async () => {
  ...
  electronApp.setAppUserModelId("gg.ktmlauncher.ktm");
  ...
  const deepLinkArg = process.argv.find((arg) =>
    arg.startsWith("ktmlauncher://")
  );
  const forceBigPicture = process.argv.includes("--big-picture");
  const isRunDeepLink = deepLinkArg?.startsWith("ktmlauncher://run");
...
app.on("second-instance", (_event, commandLine) => {
  ...
  const isRunDeepLink = deepLink?.startsWith("ktmlauncher://run");
